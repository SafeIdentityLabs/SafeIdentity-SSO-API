...
<%
	...
	if ("".equals(ssoToken)) {
		if (request.getSession().getAttribute("ssoToken") != null || !"".equals(request.getSession().getAttribute("ssoToken"))) {
			...
			System.out.println("ssoToken session : " + ssoToken);
			...
			if(!"".equals(secId)) {
				System.out.println("msso_web_sample - verifyToken secId 1");
				vResult = sso.verifyToken(ssoToken, "127.0.0.1", secId);
			} else {
				System.out.println("msso_web_sample - verifyToken no secId 1");
				vResult = sso.verifyToken(ssoToken);
			}
			System.out.println("verifyResult111 : " + vResult);

			if (vResult != 0) {
				session.invalidate();
				response.sendRedirect(request.getContextPath() + "/android/msso_auth_id_sample.jsp");
				return;
			}
		} else {
...
// 		vResult = sso.verifyToken(ssoToken);
		if(!"".equals(secId)) {
			System.out.println("msso_web_sample - verifyToken secId 2");
			vResult = sso.verifyToken(ssoToken, "127.0.0.1", secId);
		} else {
			System.out.println("msso_web_sample - verifyToken no secId 2");
			vResult = sso.verifyToken(ssoToken);
		}

		System.out.println("verifyResult : " + vResult);

		if (vResult != 0) {
			session.invalidate();
			System.out.println("msso_web_sample - vResult : " + vResult);
			response.sendRedirect(request.getContextPath() + "/android/msso_auth_id_sample.jsp");
			return;
		}
	}
	...
	if ("unreg".equals(play)) {
		System.out.println("unreg ssoToken : " + ssoToken);
...
<div data-role="page">
	...
	<div data-role="content">
		<form id="jQfrm" name="jQfrm" method="post" action="<%=request.getContextPath()%>/android/msso_web_sample.jsp">
