...
<%
	...
	//20141128 add smoh - securityID 사용유무
	boolean secIdFlag = false;
	...
	if(secIdFlag) {
		if("".equals(secId)) {
			...
			response.sendRedirect("com.softforum.sample://getSecId");
			return;
		}
		System.out.println("msso_auth_id_sample - secId : " + secId);
	}
	...
	if(!"".equals(userId) && !"".equals(userPwd)) {
		System.out.println("msso_auth_id_sample - userId : " + userId + " || userPwd : " + userPwd);
		
		SsoAuthInfo ssoAuthInfo = new SsoAuthInfo();
		//20141128 modify smoh - for securityID 사용 유무
		if(secIdFlag) {
		...
		if(ssoAuthInfo != null) {
			if(iResult >= 0) {
				...
				System.out.println("msso_auth_id_sample - ssoAuthInfo.getToken() : " + ssoAuthInfo.getToken());
				response.sendRedirect(request.getContextPath() + "/android/msso_web_sample.jsp");
				return;
			} else {
...
<div data-role="content">
	...
	<div data-role="content">
	<form id="authForm" name="authForm" method="post" action="/m/android/msso_auth_id_sample.jsp">
<% 
	if(secIdFlag) {
		out.println("<input type=\"text\" id=\"secId\" name=\"secId\" value=\"" + secId + "\" />");
	}
%>
